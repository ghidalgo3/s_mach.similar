...
class SMach_Similar_JaccardIndexTest extends FlatSpec with Matchers {
  "JaccardIndex" should "compute set similarity through implicit Similar[Traversable[A]]" in {
    ...
    Set(1,2,3) similar Set(1,2,3) should equal(1.0)
    List(0.0,0.1,0.2) similar List(1.0,2.0,3.0) should equal(0.0)
    Stream("a","b","c") similar Stream("a","b","c","d") should equal(3.0/4)
    Seq("a","b","c") similar Seq("a","a","b","c") should equal(3.0/4)
    Vector(1,2,3,5) similar Vector(1,1,1,1,2,3,4) should equal(3.0/8.0)
  }

  "JaccardIndex" should "compute set similarity commutatively through implicit Similar[Traversable[A]]" in {
    import JaccardIndex._
    List(0.0,0.1,0.2) similar List(1,2,3) should equal(List(1.0,2.0,3.0) similar List(0.0,0.1,0.2))
    Stream("a","b","c") similar Stream("a","b","c","d") should equal(Stream("a","b","c","d") similar Stream("a","b","c"))
    Seq("a","b","c") similar Seq("a","a","b","c") should equal(Seq("a","a","b","c") similar Seq("a","b","c"))
    Vector(1,2,3,5) similar Vector(1,1,1,1,2,3,4) should equal(Vector(1,1,1,1,2,3,4) similar Vector(1,2,3,5))
  }
  ...
  "JaccardIndex.DocString" should "compute string similarity using 3,4-wordgrams" in {
    ...
    gettys1 similar gettys1 should equal(1.0)
    gettys1 similar gettys2 should (be >= 0.42 and be <= 0.43)
  }
}
